pipeline {
    ...
    environment {
        ...
        FDN="visualizer.mehmetafsar.net"
    ...
    stages {
        ...
        stage('Create QA Environment Infrastructure') {
            steps {
                ...
                script {
                    ...
                    while(true) {
                        echo "Docker Worker is not UP and running yet. Will try to reach again after 2 seconds..."

                        ip = sh(script:"aws ec2 describe-instances --region ${AWS_REGION} --filters Name=tag-value,Values=worker Name=tag-value,Values=${APP_STACK_NAME} --query Reservations[*].Instances[*].[PublicIpAddress] --output text | tail -n 1", returnStdout:true).trim()

                        if (ip.length() >= 7) {
                            echo "Docker Worker Public Ip Address Found: $ip"
        ...
        stage('dns-record-control-grand-master'){
            agent any
            steps{
